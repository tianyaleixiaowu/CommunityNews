...
public class MyPushMessageReceiver extends FrontiaPushMessageReceiver {
    ...
    public void onBind(final Context context, int errorCode, String appid,
        ...
        boolean hasBind = AbSharedUtil.getBoolean(context, "hasBind", false);
        if (!hasBind) {
            HashMap map = new HashMap();

            map.put("user_id", userId);
            map.put("channel_id", channelId);
            //发送userid和channelid给服务器
            HttpUtil.sendRequest(100, map, new GetDataCallBack() {
                @Override
                public void success(String content) {
                    super.success(content);
                    //如果绑定成功的消息服务器已经接受了，下次就不再给服务器发送
                    AbSharedUtil.putBoolean(context, "hasBind", true);
                    AbSharedUtil.putString(context, "userId", userId);
                }
...
//        updateContent(context, responseString);
    ...
    private void updateContent(Context context, String content) {
        ...
        intent.putExtra("content", logText);
